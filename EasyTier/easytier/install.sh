...
install_now(){
	# default value
	local TITLE="EasyTier"
	local DESCR="国内免费开源的内网穿透"
	...
	if [ "${ARCH}" == "aarch64" ];then
		cp -rf /tmp/${module}/bin/aarch64/* /koolshare/bin/
	fi
	if [ "${ARCH}" == "armv7l" ];then
		cp -rf /tmp/${module}/bin/armv7/* /koolshare/bin/
	fi
	...
	if [ ! -L "/koolshare/init.d/S96${module}.sh" -a -f "/koolshare/scripts/${module}_config.sh" ];then
		ln -sf /koolshare/scripts/${module}_config.sh /koolshare/init.d/S96${module}.sh
	fi
	...
	chmod 755 /koolshare/init.d/S96${module}.sh >/dev/null 2>&1
	...
	# re-enable
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/${module}.sh" ];then
		echo_date "安装完毕，重新启用${TITLE}插件！"
		sh /koolshare/scripts/${module}.sh start >/dev/null 2>&1 &
	fi
