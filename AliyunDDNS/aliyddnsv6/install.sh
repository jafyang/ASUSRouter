...
platform_test(){
	...
	if [ -d "/koolshare" -a -f "/usr/bin/skipd" -a "${LINUX_VER}" -eq "41" ];then
...
install_now(){
	...
	echo "" > /tmp/upload/${module}_log.txt

	# make start up script link
	(echo -e "#!/bin/sh\n\nsh /koolshare/scripts/${module}_config.sh \$1" > /koolshare/init.d/S98${module}.sh)

	# Permissions
	chmod 755 /koolshare/scripts/${module}_*.sh >/dev/null 2>&1
	chmod 755 /koolshare/scripts/uninstall_${module}.sh >/dev/null 2>&1
	chmod 755 /koolshare/init.d/S98${module}.sh >/dev/null 2>&1
	chmod 666 /tmp/upload/${module}_log.txt >/dev/null 2>&1
	...
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/${module}_config.sh" ];then
		...
		sh /koolshare/scripts/${module}_config.sh update >/dev/null 2>&1 &
	fi
