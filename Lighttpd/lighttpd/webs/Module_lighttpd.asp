...
<html xmlns:v>
	<head>
		...
		<script>
			...
			var switchTimer = 0;
			var params = {
				enable: "lighttpd_enable",
				version: "lighttpd_core",
				status: "lighttpd_status",
				port: "lighttpd_port",
				//dualstack: "lighttpd_dualstack",
				watchdog: "lighttpd_watchdog",
				firewall: "lighttpd_firewall"
			}
			...
			function get_dbus_data() {
				$.ajax({
					...
					success: function (data) {
						...
						E(params.port).innerHTML = (!dbus[params.port] || dbus[params.port] === "") ? "无" : dbus[params.port].split(",").map(p => {
							return `<a target="_blank" href="http://<% nvram_get("lan_ipaddr"); %>:${p}" title="IPv4访问测试"><em>${p}</em></a>`;
						}).join("&nbsp;&nbsp;");
						//E(params.dualstack).checked = (dbus[params.dualstack]=="1")||false;
						E(params.watchdog).checked = (dbus[params.watchdog]=="1")||false;
						E(params.firewall).checked = (dbus[params.firewall]=="1")||false;
						update_visibility();
					},
			...
			function save() {
				...
				//new_dbus[params.dualstack] = E(params.enable).checked && E(params.dualstack).checked ? "1" : "0";
				new_dbus[params.watchdog] = E(params.enable).checked && E(params.watchdog).checked ? "1" : "0";
				new_dbus[params.firewall] = E(params.enable).checked && E(params.firewall).checked ? "1" : "0";
				var id = parseInt(Math.random() * 100000000);
				var postData = { "id": id, "method": "lighttpd_config.sh", "params": [1], "fields": new_dbus };
				...
				$(E(params.enable)).click(
					function () {
						if (E(params.enable).checked) {
							...
							E("listen_port").style.display = "";
							//E("svr_dualstack").style.display = "";
							E("svr_restart").style.display = "";
							E("svr_firewall").style.display = "";
							E("info_config").style.display = "";
						} else {
							E("core_version").style.display = "none";
							E("last_status").style.display = "none";
							E("listen_port").style.display = "none";
							//E("svr_dualstack").style.display = "none";
							E("svr_restart").style.display = "none";
							E("svr_firewall").style.display = "none";
							E("info_config").style.display = "none";
						}
					}
				);
			}
			...
			function menu_hook() {
				tabtitle[tabtitle.length - 1] = new Array("", "lighttpd");
				tablink[tablink.length - 1] = new Array("", "Module_lighttpd.asp");
			}
									...
									<table class="FormTitle" width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3">
										<tr>
											<td bgcolor="#4D595D" colspan="3" valign="top">
												<div>&nbsp;</div>
												<div class="formfonttitle" style="float:left;">Lighttpd</div>
												...
												<div style="margin-left:5px;">
													<li><a target="_blank" href="https://wiki.lighttpd.net/" title="跳转到官网"><em><u>Lighttpd</u></em></a>是一款开源Web服务器项目，有高性能、低内存开销、高处理效率及模块化架构为核心特性。</li>
												</div>
												<div style="margin:5px 0px 0px 0px;">
													<table class="FormTable" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3">
														...
														<tr id="switch_tr">
															...
															<td colspan="2">
																<div class="switch_field" style="display:table-cell">
																	<label for="lighttpd_enable">
																		<input id="lighttpd_enable" class="switch" type="checkbox" style="display:none;" />
																...
																<div id="lighttpd_version" style="padding-top:5px;margin-right:30px;margin-top:-30px;float:right;"></div>
															</td>
														</tr>
														<tr id="core_version" style="display:none;">
															<th style="cursor:help;" title="lighttpd内核发行版本号">内核版本</th>
															<td>
																<span id="lighttpd_core"></span>
															</td>
														</tr>
														<tr id="last_status" style="display:none;">
															<th>运行状态</th>
															<td>
																<span id="lighttpd_status"></span>
															</td>
														</tr>
														<tr id="listen_port" style="display:none;">
															<th>监听端口</th>
															<td>
																<span id="lighttpd_port"></span>
															</td>
														</tr>
														<!--
														<tr id="svr_dualstack" style="display:none;">
															<th style="cursor:help;" title="IPv4和IPv6双栈协议">双栈协议(支持IPv6)</th>
															<td>
																<input type="checkbox" id="lighttpd_dualstack" style="vertical-align:middle;" /><em>仅创建双栈防火墙,不影响配置</em>
															</td>
														</tr>
														-->
														<tr id="svr_restart" style="display:none;">
															...
															<td>
																<input type="checkbox" id="lighttpd_watchdog" style="vertical-align:middle;" />
															</td>
														</tr>
														<tr id="svr_firewall" style="display:none;">
															<th style="cursor:help;" title="允许外部访问可能会导致被黑客攻击">外部访问<em>(谨慎)</em></th>
															<td>
																<input type="checkbox" id="lighttpd_firewall" style="vertical-align:middle;cursor:help;" title="允许外部访问可能会导致被黑客攻击" />
															</td>
														</tr>
														<tr id="info_config" style="display:none;">
															<th style="cursor:help;" title="请使用SSH或WinSCP登录修改配置文件后重启服务">配置路径</th>
															<td>
																<span><em>/koolshare/configs/lighttpd/lighttpd.conf</em></span>
															</td>
														</tr>
													</table>
												</div>
